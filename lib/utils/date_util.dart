...
class DateUtil {
  ...
  //获取时间标题
  static String getDateTitle(int postDate){
    String dateTitle = getDateStrByMs(postDate,format: DateFormat.ZH_YEAR_MONTH_DAY);
    int differTime = (DateTime.now().millisecondsSinceEpoch - postDate)~/1000;
    if(differTime>60*60*24){
      dateTitle = "昨天" + getDateStrByMs(postDate,format: DateFormat.HOUR_MINUTE);
    }else if(differTime < 60){
      dateTitle = "刚刚";
    }else if(differTime < 60*60){
      dateTitle = (differTime~/60).toString()+"分钟前";
    }else{
      dateTitle = "今天" + getDateStrByMs(postDate,format: DateFormat.HOUR_MINUTE);
    }
